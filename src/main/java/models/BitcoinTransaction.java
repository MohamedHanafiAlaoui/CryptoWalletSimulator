...
public class BitcoinTransaction extends Transaction {
    ...
    public BigDecimal calculateFees() {
        try {
            long sizeBytesLong = Long.parseLong(sizeBytes);
            long satoshiPerByteLong = Long.parseLong(satoshiPerByte);
            return BigDecimal.valueOf(sizeBytesLong).multiply(BigDecimal.valueOf(satoshiPerByteLong));
        } catch (Exception e) {
    ...
    public void setSatoshiPerByte(String satoshiPerByte) {
        this.satoshiPerByte = satoshiPerByte;
    }
}
